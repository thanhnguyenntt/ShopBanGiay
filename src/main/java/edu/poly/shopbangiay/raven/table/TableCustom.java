...
public class TableCustom {
    ...
    public static void apply(JScrollPane scroll, TableType type) {
        ...
        table.getTableHeader().setDefaultRenderer(new table.TableHeaderCustomCellRender(table));
        table.setRowHeight(30);
        table.HoverIndex hoverRow = new table.HoverIndex();
        TableCellRenderer cellRender;
        if (type == TableType.DEFAULT) {
//            cellRender = new TableCustomCellRender(hoverRow);
        } else {
            cellRender = new table.TextAreaCellRenderer(hoverRow);
        }
//        table.setDefaultRenderer(Object.class, cellRender);
        ...
        scroll.getVerticalScrollBar().setUI(new scroll.ScrollBarCustomUI());
        scroll.getHorizontalScrollBar().setUI(new scroll.ScrollBarCustomUI());
